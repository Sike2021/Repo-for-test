...
export const Gameplay2DScreen: React.FC<Gameplay2DScreenProps> = ({ match, gameData, onMatchComplete, onExit }) => {
    ...
    const { tA, tB, pA, pB } = useMemo(() => {
        const teamA = gameData.teams.find(t => t.name === match.teamA) || gameData.teams[0];
        const teamB = gameData.teams.find(t => t.name === match.teamB) || gameData.teams[1];
        ...
        const playersA = getXI(teamA);
        const playersB = getXI(teamB);
        return { tA: teamA, tB: teamB, pA: playersA, pB: playersB };
    }, [gameData.teams, gameData.playingXIs, gameData.currentFormat, match.teamA, match.teamB]);

    const [currentInningIndex, setCurrentInningIndex] = useState(0);
    const [battingTeam, setBattingTeam] = useState<Team>(tA);
    const [bowlingTeam, setBowlingTeam] = useState<Team>(tB);
    ...
    const [modalType, setModalType] = useState<'batter' | 'bowler' | 'openers' | null>('openers');
    ...
    const [allPlayers] = useState<Player[]>(() => [...pA, ...pB]);
    ...
    const [inningsState, setInningsState] = useState<Inning[]>(() => {
        ...
        return [createInning(tA, tB, pA), createInning(tB, tA, pB)];
    });
    ...
    }, [battingTeam?.id, gameData.userTeamId, inningsState, resetBallPhase]);

    const determineShotOutcome = useCallback(() => {
        ...
        const bowlSkill = bowler.secondarySkill;
        ...
        p.ball.vz = lift;
    }, [allPlayers, handleDismissal, updateScore]);

    const processBall = useCallback(() => {
        const p = game.current;
    ...
    }, [determineShotOutcome, resetBallPhase, updateScore, handleDismissal]);
    ...
    }, [inningsState, bowlingTeam, battingTeam]);
    ...
    useEffect(() => {
        ...
        if (inn.wickets >= 10 || Math.floor(balls / 6) >= maxOvers) {
            if (currentInningIndex === 0) setTimeout(() => switchInning(), 0);
            else setTimeout(() => setGameOver(true), 0);
            return;
        }
    ...
    }, [inningsState, ballPhase, modalType, gameOver, currentInningIndex, gameData.currentFormat, target, switchInning, bowlingTeam?.id, gameData.userTeamId, bowlerId]);
    ...
    }, [gameOver, inningsState, match, onMatchComplete, target]);
    ...
    return (
        <div className="flex flex-col h-full bg-black text-white select-none overflow-hidden relative font-sans italic tracking-tighter">
            <style>{`
                ...
                .team-bg-teal { background: linear-gradient(135deg, #14b8a6 0%, #0d9488 100%); }
            `}</style>
            ...
            <div className="flex-1 relative bg-[#1c2e17] flex justify-center">
                ...
                {ballPhase === 'boundary' && (
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-ping z-50">
                        <div className="bg-teal-600 text-white font-black text-7xl px-12 py-6 border-8 border-white skew-x-[-12deg] shadow-[0_0_40px_rgba(20,184,166,0.8)]">
            ...
            <div className="bg-zinc-950 p-4 border-t border-white/5">
                     ...
                     <div className="flex gap-2">
                        {battingTeam?.id === gameData.userTeamId ? (
                            (['Defensive', 'Balanced', 'Attacking'] as Aggression[]).map(agg => (
                                <button key={agg} onClick={() => setAggression(agg)} className={`px-4 py-1.5 text-[10px] font-black rounded-full uppercase transition-all ${aggression === agg ? 'bg-teal-600 text-white shadow-[0_0_15px_rgba(20,184,166,0.4)]' : 'bg-zinc-800 text-zinc-500 hover:bg-zinc-700'}`}>{agg}</button>
                            ))
                ...
                <div className="flex gap-3 h-16">
                    {ballPhase === 'dead' && !gameOver && !modalType && (
                        <button onClick={startBall} className="flex-[3] bg-teal-600 hover:bg-teal-500 rounded-2xl font-black text-2xl italic tracking-tighter shadow-2xl transition-transform active:scale-95 animate-pulse flex items-center justify-center gap-2">
            ...
            {modalType && (
                <div className="absolute inset-0 bg-black/95 z-[100] flex items-center justify-center p-8 backdrop-blur-md">
                    <div className="w-full max-w-sm bg-zinc-900 p-8 rounded-[40px] border-2 border-teal-500/20 shadow-[0_0_60px_rgba(20,184,166,0.15)]">
                        ...
                        <div className="space-y-4">
                            <select className="w-full p-4 bg-zinc-800 rounded-2xl border-none text-white font-bold text-sm focus:ring-2 focus:ring-teal-500 appearance-none" value={selectedModalId} onChange={e => setSelectedModalId(e.target.value)}>
                            ...
                            {modalType === 'openers' && (
                                <select className="w-full p-4 bg-zinc-800 rounded-2xl border-none text-white font-bold text-sm focus:ring-2 focus:ring-teal-500 appearance-none" value={selectedModalId2} onChange={e => setSelectedModalId2(e.target.value)}>
                            ...
                            }} className="w-full bg-teal-600 hover:bg-teal-500 py-5 rounded-2xl font-black text-white shadow-xl disabled:opacity-10 uppercase italic tracking-widest text-lg transition-all mt-6">Ready to Play</button>
                        </div>
                    </div>
                </div>
            )}
