...
const Lineups: React.FC<LineupsProps> = ({ gameData, userTeam, handleUpdatePlayingXI, handleUpdateCaptain, showFeedback }) => {
    ...
    const renderPlayerCard = (player: Player, isXI: boolean, index: number) => {
        ...
        return (
            <motion.div
                ...
                className={`relative group cursor-pointer p-4 md:p-6 rounded-[24px] md:rounded-[40px] border transition-all duration-500 overflow-hidden ${
                    isSelected 
            ...
            >
                <div className={`absolute -right-4 -bottom-4 text-6xl md:text-8xl font-black italic opacity-[0.03] pointer-events-none transition-colors ${isSelected ? 'text-black' : 'text-white'}`}>
                    {index + 1}
                </div>
                ...
                <div className="flex items-center gap-4 md:gap-6 relative z-10">
                    <div className="relative">
                        ...
                        <PlayerAvatar player={player} size="sm" className={`w-14 h-14 md:w-20 md:h-20 border-2 md:border-4 relative z-10 transition-colors ${isSelected ? 'border-white/40' : 'border-white/5 group-hover:border-teal-500/30'}`} />
                        {isCaptain && (
                            <div className="absolute -top-1 -right-1 bg-yellow-500 text-black w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-[8px] md:text-[10px] font-black shadow-lg border-2 border-[#050808] z-20">
                                C
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 md:gap-3 mb-1 md:mb-2">
                            <h4 className={`text-lg md:text-2xl font-black italic uppercase tracking-tighter truncate transition-colors ${isSelected ? 'text-black' : 'text-white group-hover:text-teal-500'}`}>
                                {player.name}
                            </h4>
                            {player.isForeign && <Icons.Globe size={10} className={isSelected ? 'text-black/40' : 'text-white/20'} />}
                        </div>
                        
                        <div className="flex items-center gap-2 md:gap-3">
                            <span className={`text-[7px] md:text-[9px] font-black uppercase tracking-widest px-2 md:px-3 py-0.5 md:py-1 rounded-full border transition-colors ${
                                isSelected 
                            ...
                            {dropStatus && (
                                <span className="text-[7px] md:text-[8px] font-black text-red-500 uppercase tracking-widest bg-red-500/10 px-1.5 md:px-2 py-0.5 rounded-full border border-red-500/20">
                                    AT_RISK
                                </span>
                            )}
                        </div>
                    ...
                    <div className="text-right">
                        <p className={`text-xl md:text-3xl font-black italic leading-none mb-0.5 md:mb-1 transition-colors ${isSelected ? 'text-black' : 'text-teal-500'}`}>
                            {Math.max(player.battingSkill, player.secondarySkill)}
                        </p>
                        <p className={`text-[7px] md:text-[8px] font-black uppercase tracking-widest transition-colors ${isSelected ? 'text-black/40' : 'text-white/20'}`}>
                            RATING
                        </p>
                    </div>
                </div>
                ...
                {isXI && !isCaptain && !isSelected && (
                    ...
                    >
                        <span className="text-black text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em]">ASSIGN_CAPTAINCY</span>
                    </motion.button>
                )}
            </motion.div>
        );
    ...
    return (
        <div className="h-full flex flex-col bg-[#050808] text-[#E4E3E0] font-sans overflow-hidden relative">
            ...
            <header className="px-5 md:px-10 py-6 md:py-12 border-b border-white/5 relative overflow-hidden bg-[#050808]/40 backdrop-blur-3xl z-10">
                <div className="absolute top-0 right-0 p-4 md:p-12 opacity-[0.02] pointer-events-none">
                    <Icons.Users className="w-24 h-24 md:w-64 md:h-64" />
                </div>

                <div className="flex flex-col md:flex-row justify-between items-start gap-4 md:gap-12 mb-6 md:mb-12 relative z-10">
                    <div>
                        <motion.div 
                        ...
                        >
                            <div className="w-1 md:w-2 h-5 md:h-8 bg-teal-500 rounded-full shadow-[0_0_20px_rgba(20,184,166,0.5)]" />
                            <h2 className="text-2xl md:text-5xl font-black italic uppercase tracking-tighter text-white leading-none">SQUAD_LINEUP</h2>
                        </motion.div>
                        <div className="flex items-center gap-2 md:gap-4 ml-3 md:ml-5">
                            <select 
                                value={selectedTeamId} 
                                onChange={(e) => setSelectedTeamId(e.target.value)}
                                className="bg-transparent text-[8px] md:text-[11px] font-mono font-black text-teal-500 uppercase tracking-[0.4em] outline-none cursor-pointer hover:text-white transition-colors"
                            >
                                {gameData.teams.map(team => <option key={team.id} value={team.id} className="bg-[#050808]">{team.name.toUpperCase()}</option>)}
                            </select>
                            <span className="text-white/10">//</span>
                            <span className="text-[8px] md:text-[11px] font-mono font-black text-white/40 uppercase tracking-[0.4em]">ROSTER_MGMT</span>
                        </div>
                    </div>
                    
                    <div className="flex flex-col items-start md:items-end gap-3 md:gap-6 w-full md:w-auto">
                        <motion.div 
                            initial={{ opacity: 0, x: 20 }}
                            animate={{ opacity: 1, x: 0 }}
                            className="flex bg-white/[0.03] p-0.5 md:p-1 rounded-xl md:rounded-[24px] border border-white/10 backdrop-blur-xl w-full md:w-auto overflow-x-auto scrollbar-hide"
                        >
                            {['T20', 'List A', 'First Class'].map((cat) => (
                                <button 
                                    key={cat} 
                                    onClick={() => setCategory(cat as any)} 
                                    className={`flex-1 md:flex-none px-3 md:px-8 py-2 md:py-3 rounded-lg md:rounded-[18px] text-[7px] md:text-[10px] font-black uppercase tracking-widest transition-all duration-500 whitespace-nowrap ${
                                        category === cat 
                        ...
                        <select
                            ...
                            className="bg-white/[0.03] border border-white/10 px-3 md:px-6 py-1.5 md:py-2 rounded-full text-[7px] md:text-[10px] font-black uppercase tracking-widest text-white/60 outline-none cursor-pointer hover:bg-white/[0.08] transition-all w-full md:w-auto"
                        >
                ...
                {teamRatings && (
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 relative z-10">
                        {[
                            { label: 'STRENGTH', value: teamRatings.strength, icon: <Icons.Activity size={10} md:size={12} /> },
                            { label: 'BOWLING', value: teamRatings.bowling, icon: <Icons.Target size={10} md:size={12} /> },
                            { label: 'BATTING', value: teamRatings.batting, icon: <Icons.Zap size={10} md:size={12} /> },
                            { label: 'STARS', value: teamRatings.starPlayers, icon: <Icons.Trophy size={10} md:size={12} /> }
                        ].map(stat => (
                            <motion.div 
                                key={stat.label} 
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="bg-white/[0.03] border border-white/10 p-3 md:p-6 rounded-xl md:rounded-[24px] hover:bg-white/[0.06] transition-all duration-500 group"
                            >
                                <div className="flex items-center gap-1.5 md:gap-2 mb-1.5 md:mb-3">
                                    ...
                                    <p className="text-[7px] md:text-[9px] font-black text-white/20 uppercase tracking-widest">{stat.label}</p>
                                </div>
                                <p className="text-lg md:text-3xl font-black text-white italic tracking-tighter leading-none">{stat.value}</p>
                            </motion.div>
                        ))}
                    </div>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-5 md:p-10 scrollbar-hide relative z-10">
                {isDomesticOnlyFormat && (
                    <div className="mb-6 md:mb-12 p-4 md:p-6 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl md:rounded-[32px] flex items-center gap-3 md:gap-6">
                        <Icons.AlertTriangle className="text-yellow-500 w-5 h-5 md:w-8 md:h-8" />
                        <div>
                            <p className="text-[8px] md:text-[11px] font-black text-yellow-500 uppercase tracking-widest mb-0.5 md:mb-1">DOMESTIC_PROTOCOL_ACTIVE</p>
                            <p className="text-[7px] md:text-[10px] font-black text-white/40 uppercase tracking-widest">ONLY_LOCAL_ASSETS_PERMITTED_IN_THIS_FORMAT</p>
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-8 md:space-y-16">
                    ...
                    <section className="space-y-4 md:space-y-8">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div className="flex items-center gap-3 md:gap-6">
                                <h3 className="text-[8px] md:text-[11px] font-black uppercase tracking-[0.5em] text-white/20">PLAYING_XI</h3>
                                <div className="h-[1px] bg-white/5 w-12 md:w-32"></div>
                                <span className="text-[8px] md:text-[11px] font-black text-teal-500 italic tracking-tighter">{playingXI.length}/11</span>
                            </div>
                            <motion.button 
                                ...
                                className="bg-white/5 border border-white/10 px-3 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl text-[7px] md:text-[10px] font-black uppercase tracking-widest text-white/60 hover:bg-teal-500 hover:text-black hover:border-teal-500 transition-all duration-500 w-full sm:w-auto"
                            >
                                AUTO_SELECT_OPTIMAL
                            </motion.button>
                        </div>
                    ...
                    <section className="space-y-4 md:space-y-8">
                        <div className="flex items-center gap-3 md:gap-6">
                            <h3 className="text-[8px] md:text-[11px] font-black uppercase tracking-[0.5em] text-white/20">RESERVE_BENCH</h3>
            ...
            {/* Bottom Info Bar */}
            <div className="px-5 md:px-10 py-3 md:py-6 bg-white/[0.02] border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-3">
                <div className="flex items-center gap-2 md:gap-4">
                    <div className={`w-1.5 h-1.5 md:w-3 md:h-3 rounded-full animate-pulse ${playerToSwap ? 'bg-teal-500' : 'bg-white/20'}`} />
                    <p className="text-[7px] md:text-[10px] font-black uppercase tracking-widest text-white/40 text-center sm:text-left">
                ...
                <div className="text-center sm:text-right">
                    ...
                    <p className="text-[8px] md:text-[11px] font-black text-teal-500 uppercase tracking-widest">ROSTER_DNA_VERIFIED</p>
                </div>
            </div>
        </div>
    );
